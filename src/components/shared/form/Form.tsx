...
export default function Form({ template, btnText = 'Submit', title, handler, closeDialog }: FormProps) {
    if (closeDialog) {
        return (
            <form onSubmit={() => closeDialog()} action={handler} className="max-w-xl mx-auto bg-white padding-20 rounded-2xl shadow-lg space-y-8 d-flex f-direction-column gap-10" >
                <h4>{title}</h4>
